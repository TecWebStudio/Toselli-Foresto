...
import { getCourses, getStats, getPosts, createPost, likePost, unlikePost, getComments, createComment } from '@/lib/api';
...
// ─── Main home page ───────────────────────────────────────────
export default function HomeClient({ initialPosts, initialListings }: { initialPosts: Post[]; initialListings: Listing[] }) {
  const { user } = useAuth();
  const { t } = useLanguage();
  ...
  const [greetingKey, setGreetingKey] = useState('home.greeting_morning');
  useEffect(() => {
    const h = new Date().getHours();
    setGreetingKey(h < 12 ? 'home.greeting_morning' : h < 18 ? 'home.greeting_afternoon' : 'home.greeting_evening');
  }, []);
  ...
  useEffect(() => {
    // posts and listings are already provided server-side;
    // only fetch courses and stats client-side
    Promise.all([
      getCourses().catch(() => []),
      getStats().catch(() => null),
    ]).then(([c, s]) => {
      setCourses((c as Course[]).slice(0, 4));
      setStats(s as PlatformStats | null);
      setCoursesLoading(false);
    });
    // backfill posts/listings only if server didn't provide them
    if (initialPosts.length === 0) {
      getPosts().catch(() => []).then(p => setPosts(p as Post[]));
    }
  }, []);
